...
const createOrRetrieveACustomer = async ({
  ...
  if (error || !data?.stripe_customer_id) {
    ...
    const {error: supabaseError} = await supabaseAdmin.from("customers").upsert([{id: uuid, stripe_customer_id: customer.id}]);
...
const manageSubscriptionStatusChange = async (subscriptionId: string, customerId: string, createAction =false) => {
    ...
    const subscriptionData: Database['public']['Tables']['subscriptions']["Insert"] = {
        ...
        current_period_end: toDateTime(subscription.created).toISOString(),
    ...
    console.log(`Subscription inserted/updated: ${subscription.id} fro ${uuid}`);
